...
INTERESTING_VALUES = [
    ...
    0.0001, 0.001, 0.01, 0.1, 1.0001, 1.2345, 3.14159, 1234.56789, 9999.9999, 1e-10, 1e10, 00000000
]
...
class EdgeMutator:
    def __init__(self):
        self.strategies = ['boundary', 'type_conversion', 'injection', 'unicode', 'overflow', 'decimal']
    ...
    def mutate(self, field_name, value, field_metadata):
        ...
        if strategy == 'boundary':
            ...
            return self._decimal_mutation(field_metadata)

        return value

    def _boundary_mutation(self, metadata):
        if metadata['type'] == 'CharField' and 'max_length' in metadata:
            return random.choice([
                "A" * (metadata['max_length']),
                "A" * (metadata['max_length'] - 1),
                "A" * (metadata['max_length'] + 1)
            ])
        elif metadata['type'] in ('IntegerField', 'FloatField', 'DecimalField'):
            return random.choice([0, -1, 1, 231 - 1, -231])
        return "boundary"
    ...
    def _overflow_mutation(self, metadata):
        if metadata['type'] in ('IntegerField', 'FloatField', 'DecimalField'):
            return random.choice([2**31 - 1, -2**31, 2**63 - 1, -2**63])
        return "A" * 10000

    def _decimal_mutation(self, metadata):
        if metadata['type'] in ('FloatField', 'DecimalField'):
            return random.choice([
                0.0,
                -0.0,
                1e-10,
                ...
                Decimal('-99999999999999999999999999999.999999999999'),
                Decimal('Infinity'),
                Decimal('-Infinity'),
                Decimal('NaN')
            ])
    ...
    def execute_sequence(self, sequence):
        ...
        for operation in sequence:
            if operation == 'create':
                ...
                executed_inputs.append({"operation": "create", "input": test_case})
                result = save_via_model(test_case)

                if result.get('issue_found'):
                    issues_found.append((
                        'create',
                        {"input": test_case, "executed_inputs": executed_inputs, "sequence": sequence},
                        result
                    ))
                else:
                    self.current_data = test_case
                    try:
                        created_obj = Product.objects.filter(
                            **{k: v for k, v in test_case.items() if v is not None}
                        ).first()
                        if created_obj:
                ...
                try:
                    ...
                    read_input = self.current_data.copy()
                    executed_inputs.append({"operation": "read", "input": read_input})

                    for field, value in read_input.items():
                        if normalize(getattr(instance, field)) != normalize(value):
                            issues_found.append((
                                'read',
                                {"expected": read_input, "actual": instance.__dict__,
                                "executed_inputs": executed_inputs, "sequence": sequence},
                                {"issue_type": "DataInconsistency", "category": "bug"}
                            ))
                except Exception as e:
                    executed_inputs.append({"operation": "read", "input": {"id": self.current_id}})
                    issues_found.append((
                        'read',
                        {"id": self.current_id, "executed_inputs": executed_inputs, "sequence": sequence},
                ...
                try:
                    ...
                    fields_to_update = random.sample(list(update_data.keys()), min(2, len(update_data)))
                    for field in fields_to_update:
                        update_data[field] = smart_mutate(field, update_data[field])
                    executed_inputs.append({"operation": "update", "input": update_data})
                    ...
                    issues_found.append((
                        'update',
                        {"input": update_data, "executed_inputs": executed_inputs, "sequence": sequence},
                        {"error": str(e), "category": "crash"}
                    ))

            elif operation == 'delete':
                try:
                    executed_inputs.append({"operation": "delete", "input": {"id": self.current_id}})
                    instance = self.model_class.objects.get(id=self.current_id)
                    instance.delete()
...
# Initialize the edge mutator
edge_mutator = EdgeMutator()
...
def smart_mutate(field, val):
    ...
    # Sometimes use the edge mutator
    if random.random() < 0.5:
        return edge_mutator.mutate(field, val, field_metadata)
